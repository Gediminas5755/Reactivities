...
public class CommentHub(IMediator mediator) : Hub
{
    ...
    public override async Task OnConnectedAsync()
    {
        ...
        if (string.IsNullOrEmpty(activityId)) throw new HubException("No activityId provided");
        
        await Groups.AddToGroupAsync(Context.ConnectionId, activityId);
        var comments = await mediator.Send(new GetComments.Querry { ActivityId = activityId });
