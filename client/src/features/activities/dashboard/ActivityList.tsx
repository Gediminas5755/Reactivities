...
const ActivityList = observer(function ActivityList() {
  ...
  const { ref, inView } = useInView({
    threshold: 0.5, // half ref element see
    //triggerOnce: true
  });
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
      {activitiesGroup.pages.map((activities, index) => (
        <Box
          key={index}
          ref={index === activitiesGroup.pages.length - 1 ? ref : null} // Attach ref to the last page
          display='flex'
      ...
      {/* {activitiesGroup.pages.flatMap(page => page.items).map(activity => (
        <ActivityCard key={activity.id} activity={activity} />
      ))} */}
    </Box>
  )
